...
func (noOpWriter) WritePacket(b *Buffer, addr *net.UDPAddr) error {
	b.Release()
	return nil
}
...
type StreamToPacketWriter struct {
	Writer Writer
}

// Write implements LinkWriter.
func (w *StreamToPacketWriter) WritePacket(payload *Buffer, addr *net.UDPAddr) error {
	if payload.IsEmpty() {
		return nil
	}

	err := w.Writer.WriteMultiBuffer(MultiBuffer{payload})
	return err
}

// Dummy method.
func (w *StreamToPacketWriter) WriteMultiBuffer(mb MultiBuffer) error {
	return nil
}

type PacketToStreamWriter struct {
	Writer      LinkWriter
	Destination *net.UDPAddr
}

func (w *PacketToStreamWriter) WriteMultiBuffer(mb MultiBuffer) error {
	if mb.IsEmpty() {
		return nil
	}

	for _, b := range mb {
		err := w.Writer.WritePacket(b, w.Destination)
		if err != nil {
			return err
		}
	}
