...
func EncodeUDPPacket(request *protocol.RequestHeader, data []byte, addr *net.UDPAddr) (*buf.Buffer, error) {
	b := buf.New()
	...
	if err := addrParser.WriteAddressPort(b, dest.Address, dest.Port); err != nil {
		b.Release()
		return nil, err
	}
...
func (r *UDPReader) ReadPacket() (*buf.Buffer, *net.UDPAddr, error) {
	b := buf.New()
	if _, err := b.ReadFrom(r.reader); err != nil {
		b.Release()
		return nil, nil, err
	...
	if err != nil {
		b.Release()
		return nil, nil, nil // nil error to ignore invalid packets
	}
	return b, request.Destination().UDPAddr(), nil
}
...
func (w *UDPWriter) Write(b []byte) (int, error) {
	eb, err := EncodeUDPPacket(w.request, b, nil)
	if err != nil {
		return 0, err
	}
	defer eb.Release()
...
// Write implements buf.LinkWriter.
func (w *UDPWriter) WritePacket(payload *buf.Buffer, addr *net.UDPAddr) error {
	eb, err := EncodeUDPPacket(w.request, payload.Bytes(), addr)
	if err != nil {
		return err
	}
	defer eb.Release()
