...
func (s *Server) Process(ctx context.Context, network net.Network, conn internet.Connection, dispatcher routing.Dispatcher) error {
	...
	localHost, localPortStr, _ := net.SplitHostPort(conn.RemoteAddr().String())
	localPortInt, _ := strconv.Atoi(localPortStr)
	if list, err := proc.GetProcessesBySocket(conn.RemoteAddr().Network(), localHost, uint16(localPortInt)); err == nil {
		content.Application = list
	} else {
		content.Application = []string{"[SOCKS Proxy]"}
	}

	content.Network = conn.RemoteAddr().Network()
	content.LocalAddr = conn.RemoteAddr().String()
...
func (s *Server) handleUDPPayload(ctx context.Context, conn internet.Connection, dispatcher routing.Dispatcher) error {
	udpServer := udp.NewDispatcher(dispatcher, func(ctx context.Context, packet *udp_proto.Packet) {
		...
		udpMessage, err := EncodeUDPPacket(request, payload.Bytes(), nil)
		payload.Release()
