...
func EncodeUDPPacket(request *protocol.RequestHeader, payload []byte, addr *net.UDPAddr) (*buf.Buffer, error) {
	user := request.User
	account := user.Account.(*MemoryAccount)
...
func (v *UDPReader) ReadPacket() (*buf.Buffer, *net.UDPAddr, error) {
	buffer := buf.New()
	_, err := buffer.ReadFrom(v.Reader)
	if err != nil {
		buffer.Release()
	...
	request, payload, err := DecodeUDPPacket(v.User, buffer)
	if err != nil {
		buffer.Release()
		return nil, nil, nil // return nil error to ignore invalid packets
	}
	return payload, request.Destination().UDPAddr(), nil
}
...
func (w *UDPWriter) WritePacket(payload *buf.Buffer, addr *net.UDPAddr) error {
	packet, err := EncodeUDPPacket(w.Request, payload.Bytes(), addr)
	if err != nil {
		return err
	}
