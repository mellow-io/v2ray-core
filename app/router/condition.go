...
type CustomGeoIPMatcher struct {
	geoipCodes []string
	ipFunc     func(*Context) []net.IP
}

func NewCustomGeoIPMatcher(geoipCodes []string, onSource bool) *CustomGeoIPMatcher {
	matcher := &CustomGeoIPMatcher{
...
func (m *CustomGeoIPMatcher) Apply(ctx *Context) bool {
	ips := m.ipFunc(ctx)

	for _, ip := range ips {
		code, err := lookupGeoIP(ip)
		if err != nil {
			newError("failed to lookup GeoIP").Base(err).WriteToLog()
			return false
		}

		for _, c := range m.geoipCodes {
			if code == c {
				return true
			}
		}
...
func NewApplicationMatcher(apps []string) *ApplicationMatcher {
	pCopy := make([]string, 0, len(apps))
...
func (m *ApplicationMatcher) Apply(ctx *Context) bool {
	if ctx.Content == nil {
		return false
	}

	apps := ctx.Content.Application
	for _, matcher := range m.matchers {
		for _, app := range apps {
			if WildcardPatternMatch(app, matcher) {
				return true
			}
		}
	}
